 ...
 */
import { getEstimatedAdmiraltyCoefficientByYear, getResistanceFactorByDisplacementAndHullForm as calculateResistanceByDisplacementAndHullForm, vessels1960s, } from "./test-naval-vessels.js";
// ============================================================================
 ...
 */
function calculateEmpiricalFormula(displacement, speed, hullDragCoefficient = 0.12) {
    const baseHPCoefficient = 0.0035;
    const hullDragFactor = 1.0 + hullDragCoefficient * 8;
    const resistanceCoefficientByHullForm = calculateResistanceByDisplacementAndHullForm(displacement, displacement > 4000 ? "displacement" : "planing");
 ...
 * @returns Shaft horsepower requirement
 */
function calculateAdmiraltyFormula(displacement, speed, yearBuilt = 1960, hullForm = "displacement") {
    const kWToSHP = 0.7457;
    const admiraltyCoefficient = getEstimatedAdmiraltyCoefficientByYear(yearBuilt, displacement);
    const power = (Math.pow(displacement, 2 / 3) * Math.pow(speed, 3)) / admiraltyCoefficient / kWToSHP;
    const resistanceCoefficientByHullForm = calculateResistanceByDisplacementAndHullForm(displacement, hullForm);
 ...
 * @returns Shaft horsepower requirement
 */
function calculateAdmiraltyWithResistanceFormula(displacement, speed, yearBuilt = 1960, hullForm = "displacement", powerAdjustmentFactor = 1.0) {
...
const formulas = [
    {
        ...
        calculate: (vessel) => calculateEmpiricalFormula(vessel.displacement.fullLoad || vessel.displacement.standard, vessel.speed),
    },
    {
        name: "Formula 2: Admiralty Coefficient",
        description: "Disp^2/3 × Speed^3 / Historical Admiralty Coefficient",
        calculate: (vessel) => calculateAdmiraltyFormula(vessel.displacement.fullLoad || vessel.displacement.standard, vessel.speed, vessel.designYear, vessel.hullForm),
    },
    {
        name: "Formula 3: Admiralty + Resistance Factor",
        description: "Admiralty formula with hull efficiency adjustment",
        calculate: (vessel) => calculateAdmiraltyWithResistanceFormula(vessel.displacement.fullLoad || vessel.displacement.standard, vessel.speed, vessel.designYear, vessel.hullForm, vessel.calculations.resistanceFactor),
    },
];
function runTests() {
    console.log("╔════════════════════════════════════════════════════════════════════╗");
    ...
    for (const formula of formulas) {
        ...
        for (const vessel of vessels1960s) {
            ...
            console.log(`  ${errorIcon} ${vessel.name.substring(0, 20).padEnd(20)} | ` +
        ...
        console.log("  " + "─".repeat(80));
        console.log(`  Results: Avg ${avgError.toFixed(1)}% | Min ${minError.toFixed(1)}% | Max ${maxError.toFixed(1)}%`);
    ...
    console.log(`  ${"Ship".padEnd(20)} | ${"Cruise Speed (knots)".padStart(20)} | ${"Estimated Cruise SHP (formula)".padStart(30)} | ${"Estimated Cruise SHP (power law)".padStart(30)} | ${"Actual Cruise SHP".padStart(20)} | ${"% Error".padStart(8)}`);
    console.log("\n" + "═".repeat(180) + "\n");
    for (const vessel of vessels1960s) {
        const cruisePower = calculateAdmiraltyFormula(vessel.displacement.fullLoad || vessel.displacement.standard, vessel.cruiseSpeed || Math.floor(vessel.speed * 0.6), vessel.designYear, vessel.hullForm);
        const maxPower = calculateAdmiraltyFormula(vessel.displacement.fullLoad || vessel.displacement.standard, vessel.speed, vessel.designYear, vessel.hullForm);
        const powerRatio = Math.pow((vessel.cruiseSpeed || Math.floor(vessel.speed * 0.6)) / vessel.speed, 3);
        ...
        console.log(`  ${vessel.name.substring(0, 20).padEnd(20)} | ` +
