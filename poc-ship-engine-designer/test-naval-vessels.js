export const propulsionTypeData = {
    "steam-turbine": {
        description: "Steam turbines powered by boilers, common in mid-20th century warships. Steam turbines are less efficient at lower speeds.",
        typicalCruiseEfficiency: 0.55,
    },
    diesel: {
        description: "Diesel engines, efficient for cruising speeds. Diesel engines maintain good fuel efficiency across a range of speeds.",
        typicalCruiseEfficiency: 0.75,
    },
    cogag: {
        description: "Combined Gas and Gas - multiple gas turbines for different speed regimes. Gas turbines are efficient at high speeds but less so at cruising speeds. COGAG setups help optimize efficiency across speed ranges but is less efficient at cruising speeds then diesel engines.",
        typicalCruiseEfficiency: 0.67,
    },
    cosag: {
        description: "Combined Steam and Gas - steam turbines for cruising, gas turbines for high speed. Steam turbines are less efficient at lower speeds, making COSAG less efficient at cruising speeds compared to diesel engines.",
        typicalCruiseEfficiency: 0.6,
    },
    iep: {
        description: "Integrated Electric Propulsion - electric motors powered by various prime movers. Highly efficient across a wide speed range due to electric drive characteristics.",
        typicalCruiseEfficiency: 0.9,
    },
    codag: {
        description: "Combined Diesel and Gas - diesels for cruising, gas turbines for high speed. Diesels provide good fuel efficiency at cruising speeds, while gas turbines offer high power for sprints.",
        typicalCruiseEfficiency: 0.72,
    },
    codog: {
        description: "Combined Diesel or Gas - either diesels or gas turbines can drive the ship. Diesels are used for cruising, gas turbines for high speed.",
        typicalCruiseEfficiency: 0.75,
    },
    codad: {
        description: "Combined Diesel and Diesel - multiple diesel engines for different speed regimes. All engines are diesels, optimizing efficiency across speed ranges.",
        typicalCruiseEfficiency: 0.78,
    },
    cogog: {
        description: "Combined Gas or Gas - multiple gas turbines, only one set active at a time. Gas turbines are less efficient at cruising speeds compared to diesels.",
        typicalCruiseEfficiency: 0.65,
    },
    nuclear: {
        description: "Nuclear propulsion - nuclear reactors generate steam for turbines. Provides consistent power and efficiency across all speeds.",
 ...
 */
export function getVesselsByDisplacementRange(minDisplacement, maxDisplacement) {
    return vessels1960s.filter((v) => v.displacement.standard >= minDisplacement && v.displacement.standard <= maxDisplacement);
}
export function getVesselsByPropulsion(propulsionType) {
    return vessels1960s.filter((v) => v.propulsionType === propulsionType);
}
export function getVesselsByVesselType(vesselType) {
    return vessels1960s.filter((v) => v.vesselType === vesselType);
}
export function calculateAveragePowerPerDisplacement(vessels = vessels1960s) {
    const total = vessels.reduce((sum, v) => sum + v.calculations.powerToDisplacement, 0);
    return total / vessels.length;
}
export function getResistanceFactorByDisplacementAndHullForm(displacement, hullForm = "displacement") {
...
export function getEstimatedAdmiraltyCoefficientByYear(year, displacement) {
...
export function calculateAggregatePropulsionEfficiency(propulsionType, shaftCount = 2, hullForm = "displacement") {
    // An efficiency model taking into account propulsion type, modified by shaft count and hull form
    const propulsionEfficiencyMap = {
        "steam-turbine": 0.88,
        diesel: 0.9,
        cogag: 0.85,
        cosag: 0.82,
...
export function calculatePowerEstimate(displacement, speed, length, propulsionType = "steam-turbine") {
    // Base formula: Admiralty method with hydrodynamic adjustments
    const froudeNumber = speed / Math.sqrt(9.81 * length);
    const resistanceFactor = getResistanceFactorByDisplacementAndHullForm(displacement, "displacement");
    ...
    const estimatedPower = Math.pow(displacement, 2 / 3) * Math.pow(speed, 3) * resistanceFactor * efficiencyFactor * 0.8; // empirical scaling constant
